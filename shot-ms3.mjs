...
// клик по первой карточке шага (по её заголовку %)
await p.locator('text=/^\\d+%$/').first().click({ timeout: 6000 }).catch(async ()=>{
  await p.locator('button').filter({ hasText: '%' }).first().click({ timeout: 6000 });
});
await p.waitForTimeout(900);
await p.screenshot({ path: '/tmp/ms4-expanded.png' });
await br.close();
console.log('done');
