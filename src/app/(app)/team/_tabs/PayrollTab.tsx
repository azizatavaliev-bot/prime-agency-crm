...
import { cancelPayout, deleteBonus } from "@/lib/actions";
import { ROLES, PROJECT_ROLE } from "@/lib/constants";
import { som, monthKey, monthLabel, dateRu } from "@/lib/format";
import { Stat, Badge, Avatar, Empty } from "@/components/ui";
...
/** Ведомость зарплат — раньше отдельная страница /payroll, теперь вкладка «Команды» (только владелец). */
export default async function PayrollTab({ sp }: { sp: { month?: string; error?: string } }) {
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div className="text-sm text-muted">
          Оклад + доли со всех проектов + премии. Ставки берутся те, что действовали в этом месяце
        </div>
      ...
      {lines.length === 0 && <Empty text="Сотрудников нет — добавьте команду на вкладке «Сотрудники»" />}
