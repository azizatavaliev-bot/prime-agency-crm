...
3. Смени пароль на свой — «Команда» → своя карточка.
...
Если что-то не открывается — напиши мне.`
  ...
  return (
    <>
      ...
      {open && password && (
        <div
          className="fixed inset-0 z-[70] flex items-end justify-center bg-zinc-900/50 p-4 backdrop-blur-sm sm:items-center"
        ...
        >
          ...
          >
            <div className="mb-1 text-lg font-semibold tracking-tight">Доступ для {name}</div>
            <div className="mb-4 text-sm text-muted">
              Пароль создан. Скопируйте сообщение и отправьте сотруднику — второй раз он не
              покажется.
            </div>
            ...
            <button onClick={() => setOpen(false)} className="btn-ghost mt-2 w-full">
              Закрыть
            </button>
          </div>
        </div>
